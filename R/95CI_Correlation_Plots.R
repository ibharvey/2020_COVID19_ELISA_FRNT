...
lm.out <- lm(ORF8 ~ FRNT, data = mydata)
newx = seq(0,4096,by = 0.05)
conf_interval <- as.data.frame(predict(lm.out, newdata=data.frame(FRNT=newx), interval="confidence",
                         level = 0.95))
conf_interval$x <- newx

ggplot(mydata, aes(x=FRNT, y = ORF8)) +
  geom_point(size = 3) +
