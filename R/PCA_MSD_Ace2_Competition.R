...
colnames(cor_data) <- c("log2(FRNT50)","SARS2 Spike Trimer","SARS2 mRBD","SARS2 bRBD","SARS1 bRBD","MERS bRBD")
res <- cor(cor_data)
# 
# #ELISA on rows, Mesoscale on columns
corrplot(res,tl.col = "black", method = "number", tl.cex = 1.6, cl.cex = 1.4, cl.align.text = "l", type = "upper")
...
gplot <- ggplot(data = mydata.cm, aes(x = FRNT, y=SARS2.Trimer)) +
...
# Patient 2
df3 <- as.data.frame(t(unlist(do.call(rbind.data.frame, list(c(mydata.cm[c(11,20),])))[c("FRNT","SARS2.Trimer")])))
gplot <- gplot + geom_segment(data = df3, aes(x = FRNT1, y = SARS2.Trimer1, xend = FRNT2, yend = SARS2.Trimer2), color = "dark red", size = 4)
#gplot <- gplot + geom_text(data = df3, aes(x = x1, y = y1, label = "Patient 2"), nudge_x = 0.65, nudge_y = -0.1)

df3 <- as.data.frame(t(unlist(do.call(rbind.data.frame, list(c(mydata.cm[c(20,21),])))[c("FRNT","SARS2.Trimer")])))
gplot <- gplot + geom_segment(aes(x = FRNT1, y = SARS2.Trimer1, xend = FRNT2, yend = SARS2.Trimer2), data = df3, color = "dark red", size = 4)

df3 <- as.data.frame(t(unlist(do.call(rbind.data.frame, list(c(mydata.cm[c(21,22),])))[c("FRNT","SARS2.Trimer")])))
gplot <- gplot + geom_segment(aes(x = FRNT1, y = SARS2.Trimer1, xend = FRNT2, yend = SARS2.Trimer2), data = df3, color = "dark red", size = 4, arrow = arrow(length = unit(0.4, "inches")))


#Patient 8
df3 <- as.data.frame(t(unlist(do.call(rbind.data.frame, list(c(mydata.cm[c(6,31),])))[c("FRNT","SARS2.Trimer")])))
gplot <- gplot + geom_segment(data = df3, aes(x = FRNT1, y = SARS2.Trimer1, xend = FRNT2, yend = SARS2.Trimer2), color = "dark green", size = 4)

df3 <- as.data.frame(t(unlist(do.call(rbind.data.frame, list(c(mydata.cm[c(31,32),])))[c("FRNT","SARS2.Trimer")])))
gplot <- gplot + geom_segment(data = df3, aes(x = FRNT1, y = SARS2.Trimer1, xend = FRNT2, yend = SARS2.Trimer2), color = "dark green", size = 4, arrow = arrow(length = unit(0.4, "inches")))

#Patient 3
df3 <- as.data.frame(t(unlist(do.call(rbind.data.frame, list(c(mydata.cm[c(13,26),])))[c("FRNT","SARS2.Trimer")])))
gplot <- gplot + geom_segment(data = df3, aes(x = FRNT1, y = SARS2.Trimer1, xend = FRNT2, yend = SARS2.Trimer2), color = "dark orange", size = 4)

df3 <- as.data.frame(t(unlist(do.call(rbind.data.frame, list(c(mydata.cm[c(26,39),])))[c("FRNT","SARS2.Trimer")])))
gplot <- gplot + geom_segment(data = df3, aes(x = FRNT1, y = SARS2.Trimer1, xend = FRNT2, yend = SARS2.Trimer2), color = "dark orange", size = 4, arrow = arrow(length = unit(0.4, "inches")))


################# Plot linear
qplot <- ggplot(data = mydata.cm, aes(x = FRNT, y=SARS2.Trimer)) +
...
for(it in 20:1000)
{
  results.close <- rbind(results.close,
                         add_PRA(as.matrix(table(mydata.cm$FRNT > it,mydata.cm$SARS2.Trimer<6e6))))
  results.veryfar <- rbind(results.veryfar,
                           add_PRA(as.matrix(table(mydata.cm$FRNT > it,mydata.cm$SARS2.Trimer<1.5e6))))
}
...
plot_out3 <- ggplot() + 
  ...
  ggtitle(paste("Prediction Accuracy of Ace2 Competition"),
          subtitle = paste("SARS2 Trimer ", "[1:", dilution, "]",sep = "")) +
  theme(plot.title = element_text(size = 18, hjust = 0.5),
...
df.conv <- mydata.conv[c("SARS2.mRBD","SARS2.bRBD","SARS2.Trimer")]
...
df.neg <- mydata.neg[c("SARS2.mRBD","SARS2.bRBD","SARS2.Trimer")]
my_points.neg <- as.data.frame(predict(pca_df, df.neg))
df.mayo <- mydata.mayo[c("SARS2.mRBD","SARS2.bRBD","SARS2.Trimer")]
